...
from apps.payments.services import consume_order_cancelled, dispatch_requested_checkout
...
def dispatch_checkout_events(*, limit=20, adapter_resolver=None):
    ...
    requested_intent_ids = [
        str(intent_id)
        for intent_id in PaymentAttempt.objects.filter(status=PaymentAttempt.Status.REQUESTED).values_list(
            "intent_id", flat=True
        )
    ]
