...
from apps.integrations.exceptions import IntegrationAmbiguousError, IntegrationContractError, IntegrationPermanentError, IntegrationTransientError
...
def setup_integration():
    ...
    connection = IntegrationConnection.objects.create(organization=org, key="reference", status="active")
    egress = IntegrationEndpoint.objects.create(organization=org, connection=connection, key="orders-out", direction="egress", contract_version=1, is_active=True)
    ingress = IntegrationEndpoint.objects.create(organization=org, connection=connection, key="events-in", direction="ingress", contract_version=1, is_active=True)
...
def test_ambiguous_acceptance_never_blind_retries():
    org, _, endpoint, _ = setup_integration()
    delivery = create_delivery(organization=org, endpoint=endpoint, source_type="order", source_id="2", source_version=1, operation_key="export", idempotency_key="idem-2", payload={"scenario": "timeout"})
    dispatch_delivery(delivery.id)
    delivery.refresh_from_db()
...
def test_reconciliation_resolves_uncertain_delivery():
    org, _, endpoint, _ = setup_integration()
    delivery = create_delivery(organization=org, endpoint=endpoint, source_type="order", source_id="3", source_version=1, operation_key="export", idempotency_key="idem-3", payload={"scenario": "timeout"})
    dispatch_delivery(delivery.id)
    delivery.refresh_from_db()
...
def test_cross_organization_endpoint_is_rejected():
    ...
    with pytest.raises(IntegrationContractError):
        create_delivery(organization=other, endpoint=endpoint, source_type="order", source_id="4", source_version=1, operation_key="export", idempotency_key="idem-4", payload={})
